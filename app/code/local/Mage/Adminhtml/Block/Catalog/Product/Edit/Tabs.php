...
class Mage_Adminhtml_Block_Catalog_Product_Edit_Tabs extends Mage_Adminhtml_Block_Widget_Tabs
{
    ...
    {
        $hiddenForVendor = array("Meta Information", "Recurring Profile", "Design","Gift Options");
        $requireAttribute = array("name","description","short_description","sku","status","price","security",
                              "mintime","renttype","noticeperiod","image","small_image","thumbnail","media_gallery","gallery");
        $product = $this->getProduct();
        $roleId = Mage::helper('marketplace')->getConfig('general', 'vendor_role');
        $current_user = Mage::getSingleton('admin/session')->getUser();
        $isAdmin = !($current_user->getRole()->getRoleId() == $roleId);
      //  Mage::log('isAdmin::'. $isAdmin.'! ! isAdmin::'. !$isAdmin ,Zend_log::INFO,'layout.log',true);
        ...
        if ($setId) {
            ...
            foreach ($groupCollection as $group) {
                ...
                foreach ($attributes as $key => $attribute) {
                    ...
                    if(!$isAdmin){
                       if(!in_array($attribute->getAttributeCode(), $requireAttribute)){
                          unset($attributes[$key]);
                       }
                    }
                }
              ...
              //   Mage::log($group->getAttributeGroupName().'in array::'.(bool)in_array($group->getAttributeGroupName(), $hiddenForVendor),Zend_log::INFO,'layout.log',true);
              //   Mage::log('check::'.(in_array($group->getAttributeGroupName(), $hiddenForVendor)==1  and (bool)()),Zend_log::INFO,'layout.log',true);

                 if(!$isAdmin){
                    if(!in_array($group->getAttributeGroupName(), $hiddenForVendor)){
